 ...
 * @param inputUri - The input video URI
 * @param startTime - Start time in seconds
 * @param endTime - End time in seconds
 * @returns The trimmed video URI or null if trimming fails
 */
export async function trimVideo(
  ...
  try {
    console.log('[VideoStorage] ===== STARTING VIDEO TRIM =====');
    console.log('[VideoStorage] Input URI:', inputUri);
    console.log('[VideoStorage] Trim range:', startTime, '-', endTime, 'seconds');
    console.log('[VideoStorage] Duration:', endTime - startTime, 'seconds');

    // Generate output path
    const timestamp = Date.now();
    const outputUri = `${FileSystem.cacheDirectory}trimmed_video_${timestamp}.mp4`;
    console.log('[VideoStorage] Output URI:', outputUri);
    ...
    // -i: input file
    // -ss: start time (seeking to this position)
    // -t: duration (how long to encode)
    // -pix_fmt yuv420p: ensures compatibility with iOS Photos
    // -preset ultrafast: fast encoding
    // -c:v libx264: video codec
    // -c:a aac: audio codec
    const command = `-i "${inputUri}" -ss ${startTime} -t ${duration} -pix_fmt yuv420p -preset ultrafast -c:v libx264 -c:a aac "${outputUri}"`;
    
    console.log('[VideoStorage] FFmpeg command:', command);
    console.log('[VideoStorage] Executing FFmpeg...');

    // Execute FFmpeg command
    const session = await FFmpegKit.execute(command);
    const returnCode = await session.getReturnCode();
    
    console.log('[VideoStorage] FFmpeg execution completed');
    console.log('[VideoStorage] Return code:', returnCode);

    if (ReturnCode.isSuccess(returnCode)) {
      console.log('[VideoStorage] ✓ Video trimmed successfully');
      
      // Verify the output file exists
      const fileInfo = await FileSystem.getInfoAsync(outputUri);
      console.log('[VideoStorage] Output file info:', fileInfo);
      
      if (fileInfo.exists) {
        console.log('[VideoStorage] ✓ Output file verified, size:', fileInfo.size, 'bytes');
        console.log('[VideoStorage] ===== VIDEO TRIM COMPLETE =====');
        return outputUri;
      } else {
        console.error('[VideoStorage] ✗ Output file does not exist');
        return null;
      }
    } else {
      console.error('[VideoStorage] ✗ FFmpeg failed with return code:', returnCode);
      
      // Get error logs
      const logs = await session.getLogs();
      console.error('[VideoStorage] FFmpeg logs:');
      logs.forEach((log) => {
        console.error('[VideoStorage]', log.getMessage());
      });
      
      return null;
    }
  } catch (error) {
    console.error('[VideoStorage] ✗ Exception during video trimming:', error);
    return null;
  }
}
