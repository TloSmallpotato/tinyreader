...
export default function AllMomentsScreen() {
  ...
  useEffect(() => {
    if (!selectedChild) {
      console.log('AllMomentsScreen (iOS): No selected child, skipping subscription');
      return;
    }

    console.log('AllMomentsScreen (iOS): Setting up real-time subscription for child:', selectedChild.id);

    const momentsChannel = supabase
      .channel(`all_moments_page_ios_${selectedChild.id}`, {
        config: {
      ...
      .on(
        ...
        (payload) => {
          console.log('AllMomentsScreen (iOS): moments change detected:', payload.eventType, payload);
          fetchAllMoments();
        }
      )
      .subscribe((status, err) => {
        console.log('AllMomentsScreen (iOS): subscription status:', status);
        if (err) {
          console.error('AllMomentsScreen (iOS): subscription error:', err);
        }
      });

    return () => {
      console.log('AllMomentsScreen (iOS): Cleaning up subscription');
  ...
  const handleMomentPress = (moment: Moment) => {
    ...
    console.log('AllMomentsScreen (iOS): Trim metadata:', { trim_start: moment.trim_start, trim_end: moment.trim_end });
